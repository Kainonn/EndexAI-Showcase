...
class UpdateOrionVisualStateRequest extends FormRequest
{
    ...
    {
        $this->merge([
            'state_key' => strtoupper((string) $this->input('state_key', $this->route('orionVisualState')?->state_key)),
    ...
    {
        $stateId = $this->route('orionVisualState')?->getKey();

        return [
            'state_key' => ['required', 'string', 'max:32', 'regex:/^[A-Z0-9_]+$/', Rule::unique('orion_visual_states', 'state_key')->ignore($stateId)],
